...
```javascript
{
  ...
  receipt: {
    // The return value from web3.vns.getTransactionReceipt(hash)
...
This is promisified like all available contract instance functions, and has the same API as `web3.vns.sendTransaction` without the callback. The `to` value will be automatically filled in for you.
